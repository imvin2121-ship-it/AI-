...
async function generateLoglines() {
    ...
    try {
        const response = await fetch('http://localhost:3000/api/scripts/generate-loglines', {
            method: 'POST',
            headers: {
        ...
        if (response.ok) {
            ...
            appState.scriptData.loglines = data.loglines.slice(0, 5).map(logline => {
    ...
    step3Content.innerHTML = `
        ...
        <div class="space-y-8">
            ...
            <div>
                ...
                <textarea id="outline-editor" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary/50 min-h-[300px]" placeholder="详细描述故事的主要情节、人物关系和发展脉络...">${appState.scriptData.outline || selectedLogline}</textarea>
            </div>
...
async function generateOutline() {
    ...
    showLoading('正在生成详细故事梗概...');
    
    try {
        const response = await fetch('http://localhost:3000/api/scripts/generate-outline', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
        ...
        if (response.ok) {
            ...
            document.getElementById('outline-editor').value = data.outline;
        } else {
...
`;
        ...
        document.getElementById('outline-editor').value = mockOutline;
        showToast('当前为离线模式，显示模拟数据');
    } finally {
        hideLoading();
    }
...
async function generateEpisodes() {
    ...
    showLoading(isRegenerate ? '正在重新生成分集大纲...' : '正在批量生成分集大纲...');
    
    try {
        const response = await fetch('http://localhost:3000/api/scripts/generate-episodes', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
        ...
        if (response.ok) {
            if (isRegenerate) {
                ...
                appState.scriptData.episodes = [...appState.scriptData.episodes, ...data.episodes];
            }
            renderEpisodesStep();
        } else {
...
async function generateScript() {
    ...
    try {
        const response = await fetch('http://localhost:3000/api/scripts/generate-script', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
...
async function handleRewriteSection() {
    ...
    try {
        const response = await fetch('http://localhost:3000/api/scripts/rewrite-section', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
...
async function handleContinueScript() {
    ...
    try {
        const response = await fetch('http://localhost:3000/api/scripts/continue-script', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
